...
export const getCommentByPostId: RequestHandler<{ id: string }, {}, {}> = async (req, res, next) => {
  try {
...
export const addComment: RequestHandler<{}, {}, { text: string, post_id: string }, { userId: Types.ObjectId }> = async (req, res, next) => {
  try {
    const CommentModel = await commentModel()
